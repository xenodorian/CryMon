...
function checksum(buf: Uint8Array) {
	let s = 0;
	for (let i = 0; i < 142; i++) s = (s + buf[i]) & 0xffff;
	return s;
}
...
export function packSave(snap: SaveSnapshot): Uint8Array {
	...
	buf[252] = Math.max(0, Math.min(1, snap.activeParty ?? 0));
	buf[253] = n2;
	for (let p = 0; p < n2; p++) {
		const m = p2[p];
		const o = 156 + p * SLOT;
...
export function unpackSave(buf: Uint8Array): SaveSnapshot | null {
	...
	if (ru16(buf, 142) !== checksum(buf)) return null;
	...
	return {
		...
		activeParty: buf.length >= 253 && buf[252] ? 1 : 0,
	};
}
